...
tablas_cruzadas_excel <- function(pregunta, num_pregunta, dominios, datos,
  ...
  writeData(
    wb = wb, sheet = hojas[1], x = np, startCol = columna,
    startRow = renglon[1], borders = "none"
  )

  writeData(
    wb = wb, sheet = hojas[2], x = np, startCol = columna,
  ...
  mergeCells(
    wb = wb, sheet = hojas[1], cols = 1:ncol(f[[1]]),
    rows = (renglon[1] + 1)
  )
  ...
  mergeCells(
    wb = wb, sheet = hojas[2], cols = 1:ncol(f[[2]]),
    rows = (renglon[2] + 1)
  )
  ...
  if (tipo_pregunta == "categorica" || tipo_pregunta == "multiple") {
    ...
    formato_categorias(
      tabla = f, pregunta = pregunta, datos = datos,
